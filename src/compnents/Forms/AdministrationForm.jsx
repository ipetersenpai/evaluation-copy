...
import Options2 from "./UI/Options2";
import Skeleton from "@mui/material/Skeleton";
import JwtDecoder from "../JwtDecoder";
import { evaluate } from "../../redux/slices/evaluationSlice/evaluateSlice";
import { getUserAdminList } from "../../redux/slices/userSlice/getListOfUsersAdminSlice";
...
const AdministrationForm = () => {
  ...
  const handleSecondPage = () => {
    if (radioStates.firstPage.some(rating => rating === null) || Object.values(watchAllFields).some(value => value === null || value === undefined || value === "")) {
  ...
  const adminUserList = useSelector((state) => state.userAdmin.data);
  ...
  const onSubmit = (data) => {
    ...
    const teacher = adminUserList?.users?.find((item) => item.full_name === data.evaluated_full_name);
  ...
  useEffect(() => {
    ...
    setRadioStates((prevState) => ({...prevState, firstPage: Array(empowerment?.length).fill(null)}));
    setRadioStates((prevState) => ({...prevState, secondPage: Array(intuition?.length).fill(null)}));
    setRadioStates((prevState) => ({...prevState, thirdPage: Array(self?.length).fill(null)}));
    setRadioStates((prevState) => ({...prevState, forthPage: Array(Congruence?.length).fill(null)}));
    setRadioStates((prevState) => ({...prevState, fifthPage: Array(vision?.length).fill(null)}));
    setRadioStates((prevState) => ({...prevState, sixthPage: Array(personal?.length).fill(null)}));
    setRadioStates((prevState) => ({...prevState, seventhPage: Array(additional?.length).fill(null)}));
    }
  }, [questions, status]);
