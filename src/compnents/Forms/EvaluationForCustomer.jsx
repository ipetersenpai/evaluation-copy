...
import { officeServicesList } from "../../redux/slices/OfficeServices/officeServicesListSlice";
...
const EvaluationForCustomer = () => {
  ...
  const officeListData = useSelector((state) => state.servicesList.data.data)
...
console.log(watchAllFields)
  ...
  // filter questions
  const Availability = questions?.filter(
    (item) => item?.evaluation_type === "Availability"
  );
  const clientServices = questions?.filter(
    (item) => item?.evaluation_type === "Client Services"
  );
  const clientRelation = questions?.filter(
          ...
          {pages === "first-page" ? (
            <>
              <div className="flex flex-col lg:flex-row gap-7 outline-none sm:max-w-sm mb-24 mt-2">
                <FormControl
                  fullWidth
                  error={Boolean(errors.office_services)}
                >
