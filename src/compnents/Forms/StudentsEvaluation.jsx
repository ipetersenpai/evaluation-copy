...
const StudentsEvaluation = () => {
  ...
  const { status } = useSelector((state) => state?.evaluate);

  const loading = useSelector((state) => state?.getQuestions?.loading);
  const teacherDropDown = userList?.users;
  const {
    control,
    handleSubmit,
    register,
                      ...
                      {[1, 2, 3, 4].map((optionValue, optionIndex) => (
                        <label key={optionValue}>
                          <input
                            ...
                            checked={firstPageRadioState[index] === optionValue}
                            onChange={() =>
              ...
              <div className="flex flex-col gap-10">
                <div className="flex flex-col gap-2">
                  ...
                  <TextField
                    ...
                    rows={4}
                    defaultValue="N/a"
                    {...register("comment")} // Removed the required validation
                    aria-invalid={errors.comment ? "true" : "false"}
                  />
                  {/* The error message for required validation is removed as well */}
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <TextField
                      id="outlined-multiline-static-suggestion"
                      label="Suggestion"
                      multiline
                      error={Boolean(errors.suggestion)}
                      rows={4}
                      defaultValue="N/a"
                      {...register("suggestion")} // Removed the required validation
